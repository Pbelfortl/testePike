...
export function Card({ title, desccription, cardNumber, openCard, setOpenCard}: { title: string, desccription: string, cardNumber: number, openCard: number, setOpenCard: Function }) {
    ...
    return (
        <div className={`card transition-all duration-300 overflow-hidden ${(openCard===cardNumber) ? 'ease-in max-h-72' : 'ease-out max-h-[57px]'}`}>
            <div className={`hover:cursor-pointer font-bold flex justify-between items-center w-full `} onClick={() => {cardNumber !== openCard ? setOpenCard(cardNumber) : setOpenCard(0)}}>
